...
static wb_data_t wb_read_cfg(struct wishbone *wb, wb_addr_t addr)
{
	...
	case 16: out = ioread32(control + INTERRUPT_RANGE_START_HIGH); break;
	case 20: out = ioread32(control + INTERRUPT_RANGE_START_LOW);  break;
	case 24: out = ioread32(control + INTERRUPT_RANGE_END_HIGH);   break;
	case 28: out = ioread32(control + INTERRUPT_RANGE_END_LOW);    break;
	default: out = 0; break;
	}
...
static int probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	struct pcie_wb_dev *dev;
	unsigned char* control;
	uint32_t irq_range_start_high = 0;
	uint32_t irq_range_start_low = 0;
	uint32_t irq_range_end_high = 0;
	uint32_t irq_range_end_low = 0;
	...
	/* Get the interrupt range and print it */
	irq_range_start_high = ioread32(control + INTERRUPT_RANGE_START_HIGH);
	irq_range_start_low = ioread32(control + INTERRUPT_RANGE_START_LOW);
	irq_range_end_high = ioread32(control + INTERRUPT_RANGE_END_HIGH);
	irq_range_end_low = ioread32(control + INTERRUPT_RANGE_END_LOW);
	
	printk(KERN_ALERT PCIE_WB ": interrupt range: 0x%lx%lx-0x%lx%lx\n", (unsigned long) irq_range_start_high,
	                                                                    (unsigned long) irq_range_start_low,
	                                                                    (unsigned long) irq_range_end_high,
	                                                                    (unsigned long) irq_range_end_low);
