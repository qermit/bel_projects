...
eb_status_t eb_device_get_irq_range(eb_device_t devicep, eb_data_t* irq_start_high, eb_data_t* irq_start_low, eb_data_t* irq_end_high, eb_data_t* irq_end_low) {
  eb_cycle_t  cycle;
  eb_status_t out;
  
  /* Open a new cycle and get the whole interrupt range, see INTERRUPT_RANGE_* at pcie_wb.[h/c] for details */
  out = eb_cycle_open(devicep, 0, 0, &cycle);
  if (out != EB_OK) return out;
  
  /* Prepare the configuration read cycle */
  eb_cycle_read_config(cycle, 0x10, EB_DATA32, irq_start_high);
  eb_cycle_read_config(cycle, 0x14, EB_DATA32, irq_start_low);
  eb_cycle_read_config(cycle, 0x18, EB_DATA32, irq_end_high);
  eb_cycle_read_config(cycle, 0x1c, EB_DATA32, irq_end_low);
